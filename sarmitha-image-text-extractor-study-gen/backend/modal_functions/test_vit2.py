...
app = modal.App("test-processor2")
image = modal.Image.debian_slim().pip_install("transformers", "pillow", "numpy", "torch", "opencv-python-headless")
...
def test_processor():
    ...
    for i, img in enumerate(lines):
        print(f"Line {i} mode: {img.mode}, size: {img.size}")
